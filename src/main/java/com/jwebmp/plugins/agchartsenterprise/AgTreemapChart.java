...
/**
 * Treemap Chart builder for AG Charts Enterprise.
 *
 * @param <J> the CRTP parameter for fluent API chaining
 */
public class AgTreemapChart<J extends AgTreemapChart<J>> extends AgChart<J>
{
    ...
    public AgTreemapChart(String id)
    {
        super(id);
    }
    ...
    public Uni<AgChartOptions<?>> getInitialOptions(com.jwebmp.core.base.ajax.AjaxCall<?> call, com.jwebmp.core.base.ajax.AjaxResponse<?> response)
    {
        return Uni.createFrom().item(() -> {
            AgTreemapSeriesOptions<?> series = new AgTreemapSeriesOptions<>()
                    .setLabelKey(labelKey)
                    .setValueKey(valueKey);

            if (secondaryLabelKey != null) series.setSecondaryLabelKey(secondaryLabelKey);
            if (colorKey != null) series.setColorKey(colorKey);
            if (labelName != null) series.setLabelName(labelName);
            if (secondaryLabelName != null) series.setSecondaryLabelName(secondaryLabelName);
